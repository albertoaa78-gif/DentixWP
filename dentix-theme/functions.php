...
// ══ Clase extra en ul.products de la tienda ══════════════════════
add_filter('woocommerce_product_loop_start', function($html) {
    // Añadir clase shop-products al ul para CSS específico
    return str_replace('class="products', 'class="products shop-products', $html);
});

// ══ Quitar solo result_count y ordering de before_shop_loop ═════
// (ya los mostramos manualmente en archive-product.php)
// NO tocar shop_loop_item — son necesarios para el loop nativo de WC
add_action('after_setup_theme', function() {
...
// ══ Filtro de marca en la tienda ═════════════════════════════════
// Aplica ?filter_marca=slug a la query de WooCommerce
add_action('pre_get_posts', function($query) {
    if (is_admin() || !$query->is_main_query()) return;
    if (!is_shop() && !is_product_category() && !is_product_tag()) return;

    $filter_marca = sanitize_text_field($_GET['filter_marca'] ?? '');
    if (empty($filter_marca)) return;
